...
{
	CCritSecLock lock(m_csWatcher, TRUE);
...
**********************************************/
{
	CCritSecLock lock(const_cast<CCriticalSection&>(m_csDirWatchInfo), TRUE);
	int i;
...
**************************************************************/
{
	...
	{//completion port created/directory associated w/ it successfully
		...
		if (m_monitorThread.joinable() == false)
		{
			//start the thread
			m_monitorThread = std::thread(MonitorDirectoryChanges, this);
			...
			{
				m_dwThreadID = GetThreadId(m_monitorThread.native_handle());	// ->m_nThreadID;
...
BOOL CDirectoryChangeWatcher::UnwatchAllDirectories()
{
	...
	if (m_monitorThread.joinable())
	{
		...
		CCritSecLock lock( m_csDirWatchInfo, TRUE);
...
CDirectoryChangeWatcher::CDirWatchInfo::CDirWatchInfo(HANDLE hDir, 
 ...
 :	m_pChangeHandler( NULL ), 
	...
	m_StartStopEvent(TRUE, FALSE), //NOT SIGNALLED, MANUAL RESET
...
//
{
	...
	DWORD dwRet = ::WaitForSingleObject(m_StartStopEvent, 5000);
	...
	do{
		dwWait	= MsgWaitForMultipleObjects(1, &m_StartStopEvent.m_h, FALSE, 5000, QS_ALLINPUT);//wait five seconds
		switch( dwWait )
		{
		case WAIT_OBJECT_0:
				...
				case CDirWatchInfo::RUNNING_STATE_START_MONITORING:
					{
						...
						pdi->m_StartStopEvent.Set();//signall that the ReadDirectoryChangesW has been called
				...
				case CDirWatchInfo::RUNNING_STATE_STOP:
					{
						...
						else
						{
							...
							pdi->m_StartStopEvent.Set();//set the event that ReadDirectoryChangesW has returned and no further calls to it will be made...
						}
				...
				case CDirWatchInfo::RUNNING_STATE_STOP_STEP2:
					{
						...
						if( pdi->m_hDir == INVALID_HANDLE_VALUE )
							pdi->m_StartStopEvent.Set();//signal that no further calls to ReadDirectoryChangesW will be made
