...
bool AppFilePut::start()
{
	...
	if (td < 0) {
		...
		if (kMaxRetryCount < failcount) {
			client->app->upload_fileopen_failed(filename);
			return false;
		} else {
...
bool AppFileGet::start()
{
	enum { kMaxRetryCount = 5 };
	if (kMaxRetryCount < failcount) {
		client->app->download_fileopen_failed(m_downloadFilePath);
		return false;
	}
	...
	if ((n = client->nodebyhandle(nodehandle))) {
		...
		if (n->type == FILENODE) {
			td = client->topen(n->nodehandle, m_downloadFilePath);
			if (td < 0) {
		...
		cout << "File unavailable or removed, ";
	}
...
void DemoApp::putnodes_result(error e, targettype t, NewNode* nn, int count)
{
	delete[] nn;
...
{
	if (e != API_OK) {
		ATLTRACE("Login failed: %s\n", errorstring(e));
		cout << "Login failed: " << errorstring(e) << endl;

	} else {
		ATLTRACE("Login successful, retrieving account...\n");
