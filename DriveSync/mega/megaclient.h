...
class Request
{
	vector<std::shared_ptr<Command>> cmds;
...
typedef list<std::shared_ptr<HttpReqCommandPutFA>> putfa_list;
...
struct FileTransfer
{
	...
	std::shared_ptr<Command> pendingcmd;
	...
	string filename;
	std::wstring donwloadFilePath;	// download only
...
class CommandGetFile : public Command
{
	int td;
	int connections;
	std::wstring downloadFilePath;

public:
	void procresult();

	CommandGetFile(int t, handle h, int p, int c, const std::wstring& downloadFilePath);
};
...
public:
	...
	// start upload
	int topen(const char*, int ms = 0, int connections = 1 /*= 3*/);

	// start (partial) download
	int topen(handle, const std::wstring& downloadFilePath, const byte* = NULL, m_off_t = 0, m_off_t = -1, int = 3);
...
public:
	...
	bool dispatch(transfer_list* queue, bool force = false);
	...
	struct RequestData {
		Request			request;
		BackoffTimer	bt;	// for retry
		std::unique_ptr<HttpReq>	httpReq;
	};
	std::list<RequestData> m_uploadRequests;
	std::list<RequestData> m_downloadRequests;
	void	_RequestsExec(std::list<RequestData>& requestList);
...
struct MegaApp
{
	...
	virtual void putnodes_result(error, targettype, NewNode*, int) = 0;
	...
	virtual void upload_fileopen_failed(const std::string& filename) {};
	virtual void download_fileopen_failed(const std::wstring& downloadFilePath) {}
