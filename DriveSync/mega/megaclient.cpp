...
std::string Node::displayname_string()
{
	// not yet decrypted
	if (attrstring.size()) return "NO_KEY";
...
void MegaClient::exec()
{
	...
	do {
			...
			switch (fa->status)
			{
				case REQ_SUCCESS:
					if (fa->in.size() == sizeof(handle)) {
						...
						auto it = uhnh.lower_bound(pair<handle,handle>(h,0));
						...
						if ((n = nodebyhandle(h)) || (n = nodebyhandle(fa->th))) {
							reqs[r].add(std::make_shared<CommandAttachFA>(n->nodehandle, fa->type, fah, fa->tag));
		...
		for (auto it = fafcs.begin(); it != fafcs.end(); ) {
			...
			if (it->second->req.status != REQ_INFLIGHT && it->second->bt.armed(httpio->ds)) {
				...
				if (itf != fafs.end()) {
					...
					reqs[r].add(std::make_shared<CommandGetFA>(it->first,it->second->fahref));
		...
		_RequestsExec(m_uploadRequests);
		_RequestsExec(m_downloadRequests);
					...
					switch (pendingsc->status)
					{
						...
						case REQ_SUCCESS:
							if (*pendingsc->in.c_str() == '{') {
								...
								btsc.reset();
								break;
							} else if (atoi(pendingsc->in.c_str()) == API_ESID) {
...
void MegaClient::wait()
{
	...
	for (int i = sizeof ft / sizeof *ft; nds && i--;) {
		if (ft[i].inuse && ft[i].file)
			ft[i].bt.update(httpio->ds, &nds);
	}

	// retry failed client-server requests
	if (pendingcs == nullptr) 
		btcs.update(httpio->ds, &nds);

	// retry failed server-client requests
	if (pendingsc == nullptr && *scsn) 
		btsc.update(httpio->ds, &nds);

	// retry failed file attribute puts
	if (curfa == newfa.end()) 
		btpfa.update(httpio->ds, &nds);

	// retry failed file attribute gets
	for (fafc_map::iterator it = fafcs.begin(); it != fafcs.end(); it++) {
		if (it->second->req.status != REQ_INFLIGHT)
			it->second->bt.update(httpio->ds, &nds);
	}
...
bool MegaClient::dispatch(transfer_list* queue, bool force /*= false*/)
{
	//auto nextit = queue->end();
	...
	//			return false;
	//	} else {
	//		if ((*it)->bt.armed(httpio->ds) && 
	//			( nextit == queue->end() || 
	//			 (*it)->bt.retryin(httpio->ds) < (*nextit)->bt.retryin(httpio->ds) )) 
	//			nextit = it;
	//	}
	//}
...
void	MegaClient::_RequestsExec(std::list<RequestData>& requestList)
{
	for (auto it = requestList.begin(); it != requestList.end();) {
		// do we have an API request outstanding?
		auto pendingcs = it->httpReq.get();
		if (pendingcs) {
			switch (pendingcs->status)
			{
				case REQ_READY:
				...
				case REQ_FAILURE:	// failure, repeat with capped exponential backoff
				it->httpReq.reset();
				it->bt.backoff(httpio->ds);
				app->notify_retry(it->bt.retryin(httpio->ds));
			...
			if (it->bt.armed(httpio->ds)) {
				assert(it->request.cmdspending());
				if (it->request.cmdspending()) {
					pendingcs = new HttpReq();
					it->httpReq.reset(pendingcs);

					it->request.get(pendingcs->out);
...
{
	//int havearmed = 0;
	//for (auto& trans : *queue) {
	//	if (trans->td >= 0)
	//		return 0;

	//	if (havearmed == 0) 
	//		havearmed = trans->bt.armed(ds);
	//}
...
{
	if (getq.size()) 
		retrydeferred((transfer_list*)&getq, httpio->ds);
	if (putq.size()) 
		retrydeferred((transfer_list*)&putq, httpio->ds);
}
...
dstime MegaClient::maxdeferredtransferretrydelay(transfer_list* queue)
{
	...
	for (auto& trans : *queue) {
		if (trans->td >= 0) 
			return ~(dstime)0;
		if (trans->bt.backoff() < dsmin) 
			dsmin = trans->bt.backoff();
	}
...
{
	dstime dsget = maxdeferredtransferretrydelay((transfer_list*)&getq);
	dstime dsput = maxdeferredtransferretrydelay((transfer_list*)&putq);
...
void MegaClient::putfa(SymmCipher* filekey, handle th, fatype t, const byte* data, unsigned len)
{
	...
	newfa.emplace_back(new HttpReqCommandPutFA(this,th,t,cdata,clen));
...
int MegaClient::topen(const char* localpath, int ms, int c)
{
	...
	if (file->fopen(localpath,1,0)) {
		...
		ft[td].upload = 1;
		ft[td].pendingcmd = std::make_shared<CommandPutFile>(td, file, ms, c);
		m_uploadRequests.emplace_back();
		m_uploadRequests.back().request.add(ft[td].pendingcmd);
	...
	for (;;)
	{
		switch (client->json.getnameid())
		{
			...
			case EOO:
				...
				if (this != client->ft[td].pendingcmd.get()) return;
...
int MegaClient::topen(handle h, const std::wstring& downloadFilePath, const byte* k, m_off_t start, m_off_t len, int c)
{
	Node* n;
	int td;
	int priv;
	...
	ft[td].donwloadFilePath = downloadFilePath;

	ft[td].pendingcmd = std::make_shared<CommandGetFile>(td, h, priv, c, downloadFilePath);
	m_downloadRequests.emplace_back();
	m_downloadRequests.back().request.add(ft[td].pendingcmd);
	//reqs[r].add(ft[td].pendingcmd = std::make_shared<CommandGetFile>(td, h, priv, c));
...
CommandGetFile::CommandGetFile(int t, handle h, int p, int c, const std::wstring& downloadFilePath) :
	td(t), connections(c), downloadFilePath(downloadFilePath)
{
	cmd("g");
...
error MegaClient::setattr(Node* n, const char** newattr)
{
	...
	reqs[r].add(std::make_shared<CommandSetAttr>(this, n));
...
{
	reqs[r].add(std::make_shared<CommandPutNodes>(this, h, nullptr, newnodes, numnodes, reqtag));
}
...
{
	if (client->json.isnumeric()) 
		return client->app->putnodes_result((error)client->json.getint(),type,nn, count);
	...
	for (;;)
	{
		switch (client->json.getnameid())
		{
			...
			case EOO:
				client->applykeys();
				client->notifypurge();
				client->app->putnodes_result(e,type,nn, count);
				return;
		}
	}
}
...
void MegaClient::putnodes(const char* user, NewNode* newnodes, int numnodes)
{
	...
	if (!(u = finduser(user, 1))) 
		return app->putnodes_result(API_EARGS, USER_HANDLE, newnodes, numnodes);
...
void PubKeyActionPutNodes::proc(MegaClient* client, User* u)
{
	if (u)
	{
		...
		for (int i = nc; i--; )
		{
			if (!(t = u->pubk.encrypt((const byte*)nn[i].nodekey.data(),nn[i].nodekey.size(),buf,sizeof buf))) return client->app->putnodes_result(API_EINTERNAL,USER_HANDLE,nn, 1);

			nn[i].nodekey.assign((char*)buf,t);
		}

		client->reqs[client->r].add(std::make_shared<CommandPutNodes>(client, UNDEF, u->uid.c_str(), nn, nc, tag));
	}
	else client->app->putnodes_result(API_ENOENT,USER_HANDLE,nn, nc);
}
...
error MegaClient::rename(Node* n, Node* p)
{
	...
	if (n->setparent(p))
	{
		...
		reqs[r].add(std::make_shared<CommandMoveNode>(this, n, p));
	}
...
error MegaClient::unlink(Node* n)
{
	...
	reqs[r].add(std::make_shared<CommandDelNode>(this, n->nodehandle));
		...
		while ((name = j->getnameid()) != EOO)
		{
			switch (name)
			{
				...
				case MAKENAMEID2('t','m'):	// last modified
					tm = j->getint();		// Upload‚©‚ç‚Ì‰ž“šŽž‚ÉŒÄ‚Î‚ê‚È‚¢...
					break;
...
int MegaClient::applykeys()
{
	...
	{
		reqs[r].add(std::make_shared<CommandShareKeyUpdate>(this, &sharekeyrewrite));
		sharekeyrewrite.clear();
	}

	if (nodekeyrewrite.size())
	{
		reqs[r].add(std::make_shared<CommandNodeKeyUpdate>(this, &nodekeyrewrite));
...
void MegaClient::login(const char* email, const byte* pwkey, int nocache)
{
	...
	reqs[r].add(std::make_shared<CommandLogin>(this, email, emailhash));
}
...
void PubKeyActionSendShareKey::proc(MegaClient* client, User* u)
{
	...
	if (u && (n = client->nodebyhandle(sh)) && n->sharekey && client->checkaccess(n,OWNER))
	{
		...
		if ((t = u->pubk.encrypt(n->sharekey->key, SymmCipher::KEYLENGTH, buf, sizeof buf))) client->reqs[client->r].add(std::make_shared<CommandShareKeyUpdate>(client, sh, u->uid.c_str(), buf, t));
	}
}
...
void PubKeyActionCreateShare::proc(MegaClient* client, User* u)
{
	...
	client->restag = tag;
	client->reqs[client->r].add(std::make_shared<CommandSetShare>(client, n, u, a, newshare));
}
			...
			case MAKENAMEID2('o','k'):	// an owner key response will only occur if the same share was created concurrently with a different key
				if (client->json.storebinary(key,sizeof key+1) == SymmCipher::KEYLENGTH)
				{
					...
					if ((n = client->nodebyhandle(sh)) && n->sharekey)
					{
						...
						client->restag = tag;
						client->reqs[client->r].add(std::make_shared<CommandSetShare>(client, n, user, access, 0));
						return;
					}
				}
				break;
...
void MegaClient::queuepubkeyreq(User* u, PubKeyAction* pka)
{
	...
	else
	{
		...
		if (!u->pubkrequested) reqs[r].add(std::make_shared<CommandPubKeyRequest>(this, u));
	}
}
...
error MegaClient::invite(const char* email, visibility show)
{
	...
	reqs[r].add(std::make_shared<CommandUserRequest>(email, show));
...
void MegaClient::putua(const char* an, const byte* av, unsigned avl, int priv)
{
	...
	reqs[r].add(std::make_shared<CommandPutUA>(name.c_str(), priv ? (const byte*)data.data() : av, priv ? data.size() : avl));
}
...
void MegaClient::getua(User* u, const char* an, int p)
{
	if (an)
	{
		...
		reqs[r].add(std::make_shared<CommandGetUA>(u->uid.c_str(), name.c_str(), p));
	}
}
		...
		while (j->enterarray())
		{
			...
			if (sn && sn->sharekey && checkaccess(sn,OWNER))
			{
				...
				if (n && n->isbelow(sn))
				{
					...
					reqs[r].add(std::make_shared<CommandSingleKeyCR>(sh, nh, keybuf, n->nodekey.size()));
				}
			}
...
void MegaClient::cr_response(node_vector* shares, node_vector* nodes, JSON* selector)
{
	...
	{
		crkeys.append("\"");
		reqs[r].add(std::make_shared<CommandKeyCR>(this, &rshares, &rnodes, crkeys.c_str() + 2));
	}
}
...
{
	reqs[r].add(std::make_shared<CommandGetUserQuota>(this, ad, storage, transfer, pro));
	if (transactions) reqs[r].add(std::make_shared<CommandGetUserTransactions>(this, ad));
	if (purchases) reqs[r].add(std::make_shared<CommandGetUserPurchases>(this, ad));
	if (sessions) reqs[r].add(std::make_shared<CommandGetUserSessions>(this, ad));
}
...
void Request::add(std::shared_ptr<Command> cmd)
{
	cmds.push_back(cmd);
}
...
error MegaClient::exportnode(Node* n, int del)
{
	...
	// export node
	if (n->type == FOLDERNODE || n->type == FILENODE) reqs[r].add(std::make_shared<CommandSetPH>(this, n, del));
...
error MegaClient::openfilelink(const char* link)
{
	...
	if (Base64::atob(ptr,(byte*)&ph,NODEHANDLE)-(byte*)&ph == NODEHANDLE)
	{
		...
		if (*ptr++ == '!')
		{
			...
			{
				reqs[r].add(std::make_shared<CommandGetPH>(this, ph, key));
				return API_OK;
			}
		}
	}
...
error MegaClient::changepw(const byte* oldpwkey, const byte* newpwkey)
{
	...
	reqs[r].add(std::make_shared<CommandSetMasterKey>(this, oldkey, newkey, stringhash64(&email, &pwcipher)));
...
void MegaClient::createephemeral()
{
	...
	reqs[r].add(std::make_shared<CommandCreateEphemeralSession>(keybuf, pwbuf, sscbuf));
...
{
	reqs[r].add(std::make_shared<CommandResumeEphemeralSession>(uh, pw));
}
...
void MegaClient::sendsignuplink(const char* email, const char* name, const byte* pwhash)
{
	...
	reqs[r].add(std::make_shared<CommandSendSignupLink>(email, name, c));
}
...
{
	reqs[r].add(std::make_shared<CommandQuerySignupLink>(code, len));
}
...
{
	reqs[r].add(std::make_shared<CommandConfirmSignupLink>(code, len, emailhash));
}
...
void MegaClient::setkeypair()
{
	...
	reqs[r].add(std::make_shared<CommandSetKeyPair>((const byte*)privks.data(), privks.size(), (const byte*)pubks.data(), pubks.size()));
}
...
int MegaClient::fetchsc(DbTable* sctable)
{
	...
	while (sctable->next(&id, &data, &key)) {
		...
		{
			case CACHEDSCSN:
				if (data.size() != sizeof cachedscsn || *(handle*)data.data() != cachedscsn) 
					return 0;
				break;
...
void MegaClient::fetchnodes()
{
	...
	if (!nodes.size() && sctable && !ISUNDEF(cachedscsn) && fetchsc(sctable)) {
		...
		purgenodesusersabortsc();
		reqs[r].add(std::make_shared<CommandFetchNodes>(this));
	}
}
...
void FileTransfer::doio(MegaClient* client)
{
	...
	for (int i = connections; i--; ) {
			...
			switch (reqs[i]->status)
			{
					...
					if (upload) {
						if (reqs[i]->in.size()) {
							if (reqs[i]->in.size() == UPLOADTOKENLEN*4/3) {
								...
								if (Base64::atob(reqs[i]->in.data(),ultoken,UPLOADTOKENLEN+1)-ultoken == UPLOADTOKENLEN)
								{
									...
									client->app->transfer_update(td, size, size, starttime);
									return client->app->transfer_complete(td, uploadhandle, ultoken, filekey, &key);
								}
							}

							client->app->transfer_failed(td, (error)atoi(reqs[i]->in.c_str()));
							client->defer((transfer_list*)&client->putq, td);	// defer decryption failures indefinitely
							return client->tclose(td);
						}
					} else {
						...
						if (endpos != -1) {
							...
							if (progresscompleted >= (endpos - startblock)) 
								return client->app->transfer_complete(td, &chunkmacs, filename.c_str());
							...
							if (macsmac(&chunkmacs) == metamac) {
								...
								client->app->transfer_failed(td, filename, API_EKEY);
								client->defer((transfer_list*)&client->getq, td, 1);	// defer decryption failures indefinitely
								return client->tclose(td);
							}
						}
					}
				...
				case REQ_FAILURE:
					if (reqs[i]->httpstatus == 509) {
						// report over quota
						client->app->transfer_limit(td);

						// fixed ten-minute retry intervals
						backoff = 6000;
					} else if (client->app->transfer_error(td, reqs[i]->httpstatus, client->httpio->ds - lastdata))
						return client->tclose(td);
		...
		if (reqs[i] == nullptr || reqs[i]->status == REQ_READY) {
			...
			if (npos > pos || !size) {
				if (reqs[i] == nullptr) {
					if (upload) {
						reqs[i] = new HttpReqUL();
					} else {
						reqs[i] = new HttpReqDL();
					}
				}
	...
	if (progress != progressreported) {
		client->app->transfer_update(td, progress, size, starttime);
	...
	if ((client->httpio->ds - lastdata) >= XFERTIMEOUT) {
		...
		if (upload) {
			client->app->transfer_failed(td, API_EFAILED);
			client->defer((transfer_list*)&client->putq, td);
		} else {
			client->app->transfer_failed(td, filename, API_EFAILED);
			client->defer((transfer_list*)&client->getq, td);
		}
		client->tclose(td);
